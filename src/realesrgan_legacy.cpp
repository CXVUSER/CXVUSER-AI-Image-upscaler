...
RealESRGAN_legacy::RealESRGAN_legacy() {
...
int RealESRGAN_legacy::load(const std::string &param_path, const std::string &model_path) {
...
cv::Mat RealESRGAN_legacy::to_ocv(const cv::Mat &source, const ncnn::Mat &result) {
...
int RealESRGAN_legacy::preprocess(const cv::Mat &img, cv::Mat &pad_img, int &img_pad_h, int &img_pad_w) {
    if (img.cols % 2 != 0) {
...
int RealESRGAN_legacy::inference(const cv::Mat &in, ncnn::Mat &out, int w, int h) {
...
int RealESRGAN_legacy::tile_process(const cv::Mat &inimage, cv::Mat &outimage) {
    cv::Mat pad_inimage;
